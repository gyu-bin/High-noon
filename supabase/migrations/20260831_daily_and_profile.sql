-- Daily challenge + profile update.
-- Apply after 20260830_bounty_ranking_v3_base.sql.
...
revoke execute on function public.pvp_get_daily(text) from public;
...
begin
  ...
  insert into public.daily_completions (
    ...
    actual_result, p_player_rounds, avg_v, coalesce(p_shared, false)
  );
...
revoke execute on function public.pvp_submit_daily(text, integer[], integer, integer, text, boolean) from public;
...
revoke execute on function public.pvp_mark_daily_shared(text) from public;
grant execute on function public.pvp_mark_daily_shared(text) to anon, authenticated;

revoke all on table public.daily_challenges, public.daily_completions from anon, authenticated;
